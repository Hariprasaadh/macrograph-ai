...
class PricesAgent(SectorAgent):
    ...
    def analyze(self, data: Mapping[str, pd.DataFrame], options: Optional[Any] = None) -> SectorResponse:
        ...
        headline = direct_growth_summary(cpi, "All India General Index", start_date=start_date, end_date=end_date)
        core = direct_growth_summary(cpi, "Core CPI", start_date=start_date, end_date=end_date)
        food = direct_growth_summary(cpi, "Food", start_date=start_date, end_date=end_date)
        wpi_metric = direct_growth_summary(wpi, "ALL COMMODITIES", start_date=start_date, end_date=end_date)
        housing = series_summary(houses, start_date=start_date, end_date=end_date)
        ...
        return SectorResponse(
            agent=self.name,
            assessment=f"Consumer-price inflation is {assessment} in the latest matching source observation.",
            indicators={"headline_cpi": headline, "food_cpi": food, "core_cpi": core, "wpi": wpi_metric, "house_price_index": housing},
            alerts=alerts,
            source_metadata=sources(cpi, wpi, houses)
        )
