...
class AgricultureAgent(SectorAgent):
    ...
    def analyze(self, data: Mapping[str, pd.DataFrame]) -> SectorResponse:
        ...
        production_metric = series_summary(production, "Total Foodgrains")
        yield_metric = series_summary(yield_data, "Total Foodgrains")
        msp_metric = series_summary(msp)
        ...
        return SectorResponse(agent=self.name, assessment=f"Food-inflation supply risk is {risk} based on production, yield, and MSP signals.",
            indicators={"production": production_metric, "yield": yield_metric, "msp": msp_metric}, alerts=alerts,
