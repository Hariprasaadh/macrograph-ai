...
class NationalIncomeAgent(SectorAgent):
    ...
    def analyze(self, data: Mapping[str, pd.DataFrame], options: Optional[Any] = None) -> SectorResponse:
        ...
        real_metric = series_summary(real_gdp, "Gross Domestic Product", start_date=start_date, end_date=end_date)
        gfcf_metric = series_summary(real_gdp, "GFCF", start_date=start_date, end_date=end_date)
        nominal_metric = series_summary(current_gdp, "Gross Domestic Product", start_date=start_date, end_date=end_date)
